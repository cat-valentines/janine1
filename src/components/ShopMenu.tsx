...
export function ShopMenu({ coins, foodBalance, ownedItems, onBuy, onClose, collectibleAsset, collectibleName, onOpenMarket, onSellItems, onOpenHouse, onOpenMap, onOpenInsta, onOpenBooks, onInviteFriend }: ShopMenuProps) {
  ...
  return (
    <div className="menu-backdrop" onClick={onClose}>
      <aside className="shop-menu" onClick={(event) => event.stopPropagation()}>
        ...
        </section><section className="shop-section"><button className="shop-dropdown" onClick={onOpenHouse}><span>🏡 Your House</span><b>→</b></button><button className="invite-friend-link" onClick={onInviteFriend}>👋 Invite a friend to your House</button></section><section className="shop-section"><button className="shop-dropdown insta-menu-link" onClick={onOpenInsta}><span>📸 Insta</span><b>→</b></button></section><section className="shop-section"><button className="shop-dropdown books-menu-link" onClick={onOpenBooks}><span>📚 Book Writer</span><b>→</b></button></section></div>
      </aside>
    </div>
  );
}
