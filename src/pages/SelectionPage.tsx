...
import { groupSeenAt, groupUnread, loadGroupLatest } from '../lib/groups';
...
export function SelectionPage({ onStart }: { onStart: (selection: GameSelection) => void }) {
  ...
  const [wood, setWood] = useState(savedProfile.wood ?? 0);
  // Bought a ladder, so you can pick the apples up in the trees.
  const [hasLadder, setHasLadder] = useState(savedProfile.hasLadder ?? false);
  // Meat taken from your farm animals, to cook in the kitchen.
  const [meat, setMeat] = useState(savedProfile.meat ?? 0);
  ...
  useEffect(() => {
    saveLocalProfile({ character, setting, foodBalance, shopCoins, ownedItems, equippedItem, ownsHouse, placedFurniture, accessory, completedQuests, isMember, realName, birthday, country, houseWorld, houseFurniture, houseSeason, houseSeed, characterChosen, supplies, riddleLevel, houseSource, houseName, garden, animals, streak, daysPlayed, lastPlayed, applePantry, jewels, wood, hasLadder, meat });
  ...
  if (marketOpen) return <Suspense fallback={<main className="house-world-page"><p className="world-loading">Walking into town…</p></main>}><TownMarketPage character={character} coins={shopCoins} ownedItems={ownedItems} initialSell={path === '/market/sell'}
    ...
    onSpend={(gold) => setShopCoins((total) => Math.max(0, total - gold))}
  ...
  if (profileOpen || !characterChosen) return <ProfilePage character={character} setting={setting} firstTime={!characterChosen} ownedItems={ownedItems} onChangeCharacter={setCharacter} onChangeAccessory={setAccessory} onBuyAccessory={(id, price) => { if (shopCoins < price) return; setShopCoins((total) => total - price); setOwnedItems((items) => [...items, id]); setAccessory(id); }} onSpendCoins={(amount) => setShopCoins((c) => Math.max(0, c - amount))} onOpenHouse={() => { setWorldMode('walk'); home(); navigate('/house/world'); }} onOpenMarket={() => { home(); navigate('/market'); }} onChosen={() => { setCharacterChosen(true); makeGuestReal(); home(); }} coins={shopCoins} foodBalance={foodBalance} completedQuests={completedQuests} streak={streak} isMember={isMember} accessory={accessory} realName={realName} birthday={birthday} country={country} onSavePrivate={(fields) => { setRealName(fields.realName); setBirthday(fields.birthday); setCountry(fields.country); }} onBack={() => home()} />;
  return (
    <main className="selection-page page-shell">
      <button className="menu-button" onClick={() => setMenuOpen(true)}>☰ Menu</button>
      <button className="friends-button" onClick={() => setFriendsOpen(true)}>Friends ☺{signedIn && (Object.entries(msgLatest).some(([id, at]) => messageUnread(at, chatSeenAt(id))) || Object.entries(groupLatest).some(([gid, at]) => groupUnread(at, groupSeenAt(gid)))) && <i className="friends-unread-dot" />}</button>
