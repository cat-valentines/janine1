...
import { SongEngine, GENRES, MOODS, type Genre, type Mood, type SongSpec } from '../game/songEngine';
...
export function SongStudioPage({ onScore, onBack }: { onScore: (coins: number) => void; onBack: () => void }) {
  ...
  const spec = (): SongSpec => ({ genre, mood, tempo, seed, bars: 8 });
  ...
  const newBeat = () => { const s = rand(); setSeed(s); stopAll(); setTimeout(() => { engine.current?.play({ genre, mood, tempo, seed: s, bars: 8 }, voice, () => setPlaying(false)); setPlaying(true); }, 40); };
  ...
  const save = async () => {
    ...
    try {
      const song = await createSong(userId, { title: title.trim(), genre, mood, tempo, seed, bars: 8, lyrics, is_public: isPublic });
  ...
  const playSong = (song: Song) => {
    ...
    else { engine.current?.play({ genre: song.genre, mood: song.mood, tempo: song.tempo, seed: Number(song.seed), bars: song.bars }, null, () => setNowPlaying('')); setNowPlaying(song.id); }
  };
  ...
  const loadSong = (song: Song) => { setGenre(song.genre); setMood(song.mood); setTempo(song.tempo); setSeed(Number(song.seed)); setLyrics(song.lyrics); setTitle(song.title); setIsPublic(song.is_public); setVoice(null); setTab('make'); stopAll(); };
  ...
  const songRow = (song: Song, mineTab: boolean) => <div className="song-row" key={song.id}>
    ...
    <div className="song-meta" onClick={() => loadSong(song)}>
      ...
      <small>{GENRES.find((g) => g.id === song.genre)?.icon} {song.genre} · {MOODS.find((m) => m.id === song.mood)?.icon} {song.mood}{song.audio_path ? ' · 🎙️' : ''}{!mineTab && <> · {iconOf(song.owner_id)} @{nameOf(song.owner_id)}</>}</small>
    </div>
    {mineTab
